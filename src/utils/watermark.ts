 ...
 * Read environment variable NEXT_PUBLIC_APP_BUILD_HASH from Next.js config build-time injection,
 * or __APP_BUILD_HASH__ injected by older build systems,
 * or fallback to NEXTGENOS.version.
 */
export const APP_BUILD_VERSION =
  process.env.NEXT_PUBLIC_APP_BUILD_HASH ||
...
export function clearStaleState() {
  ...
  try {
    ...
    console.error('[VersionGate] Error clearing sessionStorage:', sessErr);
  }

  // 4. Purge browser Cache Storage API
  if (typeof window !== 'undefined' && 'caches' in window) {
    try {
      caches.keys().then((keys) => {
        return Promise.all(keys.map((key) => caches.delete(key)));
      }).catch((cacheErr) => {
        console.error('[VersionGate] Cache storage keys fetch/delete failed:', cacheErr);
      });
    } catch (cacheErr) {
      console.error('[VersionGate] Cache storage deletion error:', cacheErr);
    }
  }
}
