...
REVOKE ALL ON FUNCTION emulate_stripe_webhook(UUID, TEXT, TEXT) FROM PUBLIC, anon, authenticated;

CREATE OR REPLACE FUNCTION consume_public_order_attempt(target_store_id TEXT, target_ip_hash TEXT, max_attempts INTEGER, window_minutes INTEGER)
...
BEGIN
  ...
  DELETE FROM public_order_rate_limits WHERE created_at < now() - make_interval(mins => window_minutes);
  SELECT count(*) INTO attempt_count FROM public_order_rate_limits
   WHERE store_id = target_store_id AND ip_hash = target_ip_hash
     AND created_at >= now() - make_interval(mins => window_minutes);
  IF attempt_count >= max_attempts THEN RETURN false; END IF;
  INSERT INTO public_order_rate_limits(store_id, ip_hash) VALUES (target_store_id, target_ip_hash);
  RETURN true;
END $$;
REVOKE ALL ON FUNCTION consume_public_order_attempt(TEXT, TEXT, INTEGER, INTEGER) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION consume_public_order_attempt(TEXT, TEXT, INTEGER, INTEGER) TO service_role;

CREATE TABLE IF NOT EXISTS staff_pin_rate_limits (id BIGSERIAL PRIMARY KEY, store_id TEXT NOT NULL, ip_hash TEXT NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now());
ALTER TABLE staff_pin_rate_limits ENABLE ROW LEVEL SECURITY;

CREATE OR REPLACE FUNCTION consume_staff_pin_attempt(target_store_id TEXT, target_ip_hash TEXT, max_attempts INTEGER, window_minutes INTEGER)
RETURNS BOOLEAN LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE attempt_count INTEGER;
BEGIN
  PERFORM pg_advisory_xact_lock(hashtextextended(target_store_id || ':' || target_ip_hash, 0));
  DELETE FROM staff_pin_rate_limits WHERE created_at < now() - make_interval(mins => window_minutes);
  SELECT count(*) INTO attempt_count FROM staff_pin_rate_limits
   WHERE store_id = target_store_id AND ip_hash = target_ip_hash
     AND created_at >= now() - make_interval(mins => window_minutes);
  IF attempt_count >= max_attempts THEN RETURN false; END IF;
  INSERT INTO staff_pin_rate_limits(store_id, ip_hash) VALUES (target_store_id, target_ip_hash);
  RETURN true;
END $$;
REVOKE ALL ON FUNCTION consume_staff_pin_attempt(TEXT, TEXT, INTEGER, INTEGER) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION consume_staff_pin_attempt(TEXT, TEXT, INTEGER, INTEGER) TO service_role;
