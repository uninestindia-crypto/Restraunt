/**
 * PASTE-READY COPY — generated from supabase/functions/public-order/index.ts
 *
 * The Supabase dashboard editor scopes a function to its own folder, so the shared-helper import
 * in the real entrypoint cannot resolve there. This file is that entrypoint with the two helpers
 * it uses (corsHeaders, jsonResponse) inlined, so it can be pasted into the dashboard as one file.
 *
 * The source of truth is still supabase/functions/public-order/index.ts, and
 * `npx supabase functions deploy public-order` deploys that one. This copy exists only for a
 * dashboard deploy. tests/dashboard_copy_in_sync.test.ts fails if the two drift apart on anything
 * that matters — a stale paste-ready copy is worse than no copy at all.
 */

// @ts-ignore: Deno import
import { createClient } from "https://esm.sh/@supabase/supabase-js@2";
// Inlined helpers (the dashboard cannot resolve the shared module).
const corsHeaders = {
...
function jsonResponse(body: unknown, status = 200, headers: Record<string, string> = corsHeaders) {
